...
export class ImageGalleryComponent implements OnInit, OnDestroy {
  ...
  constructor(public apiService: ApiService) { } // public for template access

  ngOnInit(): void {
    this.loadAvailableFolders();
    this.loadImages(); // Initial load
  }
  ...
  onImageError(event: Event, image: ImageInfo) {
    ...
    // For simplicity, we'll just let it show the browser's broken image icon
    // imgElement.src = 'path/to/placeholder.png'; 
    // imgElement.style.border = '1px dashed red'; // Visual cue
    // To hide the item completely if image fails (can be aggressive):
    // const parentItem = imgElement.closest('.image-item');
    // if (parentItem) (parentItem as HTMLElement).style.display = 'none';
  }
